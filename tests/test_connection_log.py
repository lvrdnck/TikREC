...
class ConnectionLogTests(unittest.TestCase):
    def test_raw_arrivals_does_not_shift_legacy_positional_fields(self):
        record = ConnectionRecord(
            1, 1.0, 2.0, None, (), "closed", None, (), None,
            3.0, 4.0, 5.0, 6.0, 7.0, "hd1", "flv_pull_url",
        )

        self.assertEqual((record.resolved_at, record.rendition_source), (3.0, "flv_pull_url"))
        self.assertIsNone(record.raw_arrivals)
    ...
    def test_old_style_record_keeps_new_fields_unknown(self):
        ...
        for field in ("raw_arrivals", "resolved_at", "http_opened_at", "first_media_tag_at",
                      "first_retained_media_at", "last_retained_media_at", "rendition_label",
                      "rendition_source"):
